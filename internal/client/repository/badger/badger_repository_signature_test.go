...
func TestBadgerRepository_Signature(t *testing.T) {
	...
	reqHash := common.HexToHash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
	...
	t.Run("SaveSignature and GetSignatureByIndex", func(t *testing.T) {
		err := repo.DoUpdateInTx(context.Background(), func(ctx context.Context) error {
			return repo.SaveSignature(ctx, reqHash, 5, sig1)
		})
		require.NoError(t, err)

		retrievedSig, err := repo.GetSignatureByIndex(context.Background(), reqHash, 5)
		require.NoError(t, err)
		require.Equal(t, sig1, retrievedSig)
	})

	t.Run("GetSignatureByIndex - not found", func(t *testing.T) {
		_, err := repo.GetSignatureByIndex(context.Background(), reqHash, 999)
		require.Error(t, err)
	...
	t.Run("GetAllSignatures - multiple signatures", func(t *testing.T) {
		err := repo.DoUpdateInTx(context.Background(), func(ctx context.Context) error {
			if err := repo.SaveSignature(ctx, reqHash, 10, sig1); err != nil {
				return err
			}
			return repo.SaveSignature(ctx, reqHash, 20, sig2)
		})
		require.NoError(t, err)

		signatures, err := repo.GetAllSignatures(context.Background(), reqHash)
		require.NoError(t, err)
		...
		retrievedSig1, err := repo.GetSignatureByIndex(context.Background(), reqHash, 10)
		require.NoError(t, err)
		require.Equal(t, sig1, retrievedSig1)

		retrievedSig2, err := repo.GetSignatureByIndex(context.Background(), reqHash, 20)
		require.NoError(t, err)
...
func TestBadgerRepository_SignatureOrdering(t *testing.T) {
	...
	reqHash := common.HexToHash("0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890")
	...
	err := repo.DoUpdateInTx(context.Background(), func(ctx context.Context) error {
		for i, index := range testIndices {
			sig := entity.SignatureExtended{
				...
				PublicKey:   []byte{byte(i)}, // Different public key for each
			}
			if err := repo.SaveSignature(ctx, reqHash, index, sig); err != nil {
				return err
			}
		}
		return nil
	...
	for i, expectedIndex := range expectedOrder {
		retrievedSig, err := repo.GetSignatureByIndex(context.Background(), reqHash, expectedIndex)
