...
func keyLatestPendingValidatorSet() []byte {
	return []byte("latest_pending_validator_set")
}
...
func (r *Repository) SavePendingValidatorSet(_ context.Context, reqHash common.Hash, vs entity.ValidatorSet) error {
	...
	return r.db.Update(func(txn *badger.Txn) error {
		...
		if err = txn.Set(keyPendingValidatorSet(reqHash), bytes); err != nil {
			return errors.Errorf("failed to store pending validator set: %w", err)
		}

		if err = txn.Set(keyLatestPendingValidatorSet(), bytes); err != nil {
			return errors.Errorf("failed to store latest pending validator set: %w", err)
		}
...
func (r *Repository) GetLatestPendingValidatorSet(_ context.Context) (entity.ValidatorSet, error) {
	var vs entity.ValidatorSet

	return vs, r.db.View(func(txn *badger.Txn) error {
		item, err := txn.Get(keyLatestPendingValidatorSet())
		if err != nil {
			if errors.Is(err, badger.ErrKeyNotFound) {
				return errors.Errorf("no latest pending validator set found: %w", entity.ErrEntityNotFound)
			}
			return errors.Errorf("failed to get latest pending validator set: %w", err)
		}

		value, err := item.ValueCopy(nil)
		if err != nil {
			return errors.Errorf("failed to copy latest pending validator set value: %w", err)
		}

		vs, err = bytesToValidatorSet(value)
		if err != nil {
			return errors.Errorf("failed to unmarshal latest pending validator set: %w", err)
		}
