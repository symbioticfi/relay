...
type missingSignatureRepo struct {
	repo

	lookups int
}

func (r *missingSignatureRepo) GetSignatureByIndex(context.Context, common.Hash, uint32) (symbiotic.Signature, error) {
	r.lookups++
	return symbiotic.Signature{}, entity.ErrEntityNotFound
}
func TestWantSignaturesBoundsMissingLookups(t *testing.T) {
	bitmap := entity.NewBitmap()
	bitmap.AddRange(0, 1000000)
	bitmap.RunOptimize()
	repository := &missingSignatureRepo{}
