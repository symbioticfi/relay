...
func createTestData(signatureTargetID common.Hash, epoch uint64, totalValidators, signers int, key crypto.PrivateKey) testData {
	// Create validators
	...
	signatureMap := entity.NewSignatureMap(signatureTargetID, entity.Epoch(epoch), uint32(totalValidators))
...
func createTestDataWithQuorum(signatureTargetID common.Hash, epoch uint64, thresholdReached bool, key crypto.PrivateKey) testData {
	if thresholdReached {
		// 8 signers * 100 voting power = 800 > 670 threshold
		return createTestData(signatureTargetID, epoch, 10, 8, key)
	}

	// 6 signers * 100 voting power = 600 < 670 threshold
	return createTestData(signatureTargetID, epoch, 10, 6, key)
}
...
func setupSuccessfulAggregationMocks(setup *testSetup, msg entity.SignatureExtended, testData testData) {
	...
	setup.mockRepo.EXPECT().GetSignatureMap(gomock.Any(), msg.SignatureTargetID()).Return(testData.SignatureMap, nil)
	setup.mockRepo.EXPECT().GetValidatorSetByEpoch(gomock.Any(), uint64(msg.Epoch)).Return(testData.ValidatorSet, nil)
	setup.mockRepo.EXPECT().GetAllSignatures(gomock.Any(), msg.SignatureTargetID()).Return(signatures, nil)
	setup.mockRepo.EXPECT().GetConfigByEpoch(gomock.Any(), uint64(msg.Epoch)).Return(networkConfig, nil)
	setup.mockRepo.EXPECT().GetAggregationProof(gomock.Any(), msg.SignatureTargetID()).Return(entity.AggregationProof{}, entity.ErrEntityNotFound)
...
func TestHandleSignatureGeneratedMessage_LowLatencyPolicy_QuorumNotReached(t *testing.T) {
	...
	testingData := createTestDataWithQuorum(msg.SignatureTargetID(), uint64(msg.Epoch), false, setup.privateKey)

	setup.mockRepo.EXPECT().GetSignatureMap(gomock.Any(), msg.SignatureTargetID()).Return(testingData.SignatureMap, nil)
...
func TestHandleSignatureGeneratedMessage_LowLatencyPolicy_QuorumReached(t *testing.T) {
	...
	testingData := createTestDataWithQuorum(msg.SignatureTargetID(), uint64(msg.Epoch), true, setup.privateKey)
...
func TestHandleSignatureGeneratedMessage_LowCostPolicy_QuorumReached_TooManyUnsigners(t *testing.T) {
	...
	testingData := createTestData(msg.SignatureTargetID(), uint64(msg.Epoch), 10, 7, setup.privateKey)
...
func TestHandleSignatureGeneratedMessage_LowCostPolicy_QuorumReached_AcceptableUnsigners(t *testing.T) {
	...
	testingData := createTestData(msg.SignatureTargetID(), uint64(msg.Epoch), 10, 8, setup.privateKey)
...
func TestHandleSignatureGeneratedMessage_LowCostPolicy_AllValidatorsSigned(t *testing.T) {
	...
	testingData := createTestData(msg.SignatureTargetID(), uint64(msg.Epoch), 10, 10, setup.privateKey)
...
func TestHandleSignatureGeneratedMessage_LowCostPolicy_ZeroMaxUnsigners(t *testing.T) {
	...
	testingData := createTestData(msg.SignatureTargetID(), uint64(msg.Epoch), 5, 4, setup.privateKey)
...
func TestSignatureMapFunctionality(t *testing.T) {
	signatureTargetID := common.HexToHash("0x123")

	// Test with unified creation
	pk, err := crypto.GeneratePrivateKey(entity.KeyTypeBlsBn254)
	require.NoError(t, err)
	testingData := createTestData(signatureTargetID, 1, 5, 0, pk) // 5 validators, 0 signers initially
