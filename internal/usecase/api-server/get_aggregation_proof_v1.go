...
func (h *grpcHandler) GetAggregationProof(ctx context.Context, req *v1.GetAggregationProofRequest) (*v1.AggregationProof, error) {
	proof, err := h.cfg.Repo.GetAggregationProof(ctx, common.HexToHash(req.RequestHash))
	if err != nil {
		return nil, errors.Errorf("failed to get aggregation proof: %w", err)
	}

	return &v1.AggregationProof{
