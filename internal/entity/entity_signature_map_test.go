...
func TestSignatureMap_SetValidatorPresent(t *testing.T) {
	...
	setupSignatureMap := func() (*SignatureMap, symbiotic.Validator, symbiotic.Validator) {
		...
		activeValidator1 := symbiotic.Validator{
			Operator:    operator1,
			VotingPower: symbiotic.ToVotingPower(big.NewInt(100)),
			IsActive:    true,
		}

		activeValidator2 := symbiotic.Validator{
			Operator:    operator2,
...
func TestSignatureMap_ThresholdReached(t *testing.T) {
	...
	t.Run("returns false when current voting power is below threshold", func(t *testing.T) {
		vm := &SignatureMap{
			CurrentVotingPower: symbiotic.ToVotingPower(big.NewInt(500)),
		}
	...
	t.Run("returns true when current voting power exceeds threshold", func(t *testing.T) {
		vm := &SignatureMap{
			CurrentVotingPower: symbiotic.ToVotingPower(big.NewInt(1500)),
		}
		quorumThreshold := symbiotic.ToVotingPower(big.NewInt(1000))
	...
	t.Run("handles zero threshold", func(t *testing.T) {
		...
		quorumThreshold := symbiotic.ToVotingPower(big.NewInt(0))
	...
	t.Run("handles large numbers", func(t *testing.T) {
		...
		vm := &SignatureMap{
			CurrentVotingPower: symbiotic.ToVotingPower(largeVotingPower),
		}
		quorumThreshold := symbiotic.ToVotingPower(largeThreshold)
...
func TestSignatureMap_IntegrationScenarios(t *testing.T) {
	...
	t.Run("edge case - exactly 100% participation", func(t *testing.T) {
		...
		validators := symbiotic.Validators{
			{
				Operator:    common.HexToAddress("0x1111111111111111111111111111111111111111"),
				VotingPower: symbiotic.ToVotingPower(big.NewInt(250)),
				IsActive:    true,
			},
			{
				Operator:    common.HexToAddress("0x2222222222222222222222222222222222222222"),
		...
		require.NoError(t, err)
		assert.True(t, vm.ThresholdReached(quorumThreshold))
		assert.Equal(t, symbiotic.ToVotingPower(big.NewInt(500)), vm.CurrentVotingPower)
	})
}
...
func TestSignatureMap_GetMissingValidators(t *testing.T) {
	...
	t.Run("returns all validators when none are present", func(t *testing.T) {
		requestID := common.HexToHash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
		vm := NewSignatureMap(requestID, symbiotic.Epoch(1), 4)
	...
	t.Run("returns subset when some validators are present", func(t *testing.T) {
		...
		err := vm.SetValidatorPresent(0, symbiotic.ToVotingPower(big.NewInt(100)))
		require.NoError(t, err)
		err = vm.SetValidatorPresent(2, symbiotic.ToVotingPower(big.NewInt(200)))
		require.NoError(t, err)
	...
	t.Run("returns empty when all validators are present", func(t *testing.T) {
		requestID := common.HexToHash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
		vm := NewSignatureMap(requestID, symbiotic.Epoch(1), 3)

		// Set all validators as present
		for i := uint32(0); i < 3; i++ {
			err := vm.SetValidatorPresent(i, symbiotic.ToVotingPower(big.NewInt(100)))
			require.NoError(t, err)
		}
	...
	t.Run("handles single validator", func(t *testing.T) {
		requestID := common.HexToHash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
		vm := NewSignatureMap(requestID, symbiotic.Epoch(1), 1)
