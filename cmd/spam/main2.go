...
func main() {
	...
	for k := 0; k < 1000; k++ {
		...
		// Sign message and wait for aggregation via streaming
		for i, relayClient := range relayClients {
			go func() {
				...
				// Use SignMessageAndWait streaming API
				stream, err := relayClient.SignMessageAndWait(context.Background(), uint32(15), messageToSign, &epoch.Epoch)
				if err != nil {
					log.Printf("Failed to sign and wait, node %d: %v, took: %f seconds", i, err, time.Since(start).Seconds())
					return
				}
				...
				for {
					...
					if err != nil {
						log.Printf("Stream error from node %d: %v, took: %f seconds", i, err, time.Since(start).Seconds())
						return
					}
					...
					case client.SigningStatus_SIGNING_STATUS_PENDING:
						log.Printf("Node %d: request %s is pending", i, requestID)
						receivedRequestIds += 1
					case client.SigningStatus_SIGNING_STATUS_COMPLETED:
						log.Printf("Successfully got aggregation proof from Relay node %d, request id: %s, proof length %d in %f seconds\n",
							i, requestID, len(resp.GetAggregationProof().Proof), time.Since(start).Seconds())
					...
					case client.SigningStatus_SIGNING_STATUS_TIMEOUT:
						log.Printf("Timeout waiting for aggregation proof from node %d, request id: %s in %f seconds\n",
							i, requestID, time.Since(start).Seconds())
						return
					default:
						log.Printf("Unknown status %v from node %d, request id: %s", resp.Status, i, requestID)
						return
					}
				}
			}()
